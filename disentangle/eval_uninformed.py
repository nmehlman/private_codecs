...
if __name__ == "__main__":
    ...
    for i, sample in tqdm.tqdm(enumerate(dataset), total=len(dataset), desc="Exporting Data"):
        ...
        with torch.no_grad():
            ...
            embedding_private, _ = pl_model(quantized_embedding, emotion_embedding[f"{emotion_model}_embedding"]) # CHANGEME
            codes_private, _ = codec.quantize(embedding_private)
            audio_private = codec.decode(codes_private)

        with torch.no_grad():
            embedding_self_recon, _ = pl_model(quantized_embedding, emotion_embedding[f"{emotion_model}_embedding"]) # DEBUG
